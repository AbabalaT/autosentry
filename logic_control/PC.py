...
target_spinning_speed = 18000  # 期望小陀螺速度
    ...
    while not rospy.is_shutdown():
        ...
        if seq_1 >= 5:
            ...
            if (game_status == 4) and (require_add_HP == 1):
                ...
                target_pose.pose.position.y = -1.86
                target_pose.pose.position.z = 0
            elif (game_status == 4) and (robot_HP > 300) and (attack_cnt > 0):
                target_pose.pose.position.x = 6.23
                target_pose.pose.position.y = -2.66
                target_pose.pose.position.z = 0
            elif (game_status == 4) and (base_HP > 500):
                if remain_time > 220:
                    target_pose.pose.position.x = 6.6
                    target_pose.pose.position.y = -2.45
